<template>
     <div class="opinion-weibo" ref="weibo">
      <div class="tabs" ref="weiboTop">
      ...
      <div class="content" ref="weiboContent">
...
export default { 
    ...
    beforeRouteLeave(to, from, next) {
            if(to.path !== '/detail'){
                this.$store.commit('WEIBO_PARAM',{filterValue:1,sortValue:'timedown',timeValue:'all',trendValue:'all',keyword:'',carry:'微博',page:1,sortIndex:2})
                next()
            } else {
                next()
            }
    ...
    methods:{  
       ...
       _calcHeight() {
        const watch = this.$refs.weibo.getBoundingClientRect()
        const top = this.$refs.weiboTop.getBoundingClientRect()
        const picHeight = watch.height - top.height
        const content = this.$refs.weiboContent
        content.style.height = picHeight + 'px'
      },
      async loadMore(){    
          const param = Object.assign({}, this.param, {page:this.param.page+1})
          this.$store.commit('WEIBO_PARAM',param)
