<template>
  <div class="opinion-material" ref="collect">
    ...
    <div class="list-wrapper" v-if="!isBlankpageShow" ref="collectContent">
      <ul>
...
<script type="text/ecmascript-6">
  ...
  import Blankpage from '@/assets/webwxgetmsgimg1.jpg'
  ...
  export default {
    ...
    methods: {
      ...
      _calcHeight() {
        const watch = this.$refs.collect.getBoundingClientRect()
        const picHeight = watch.height
        const content = this.$refs.collectContent
      ...
      loadMore() {
        this.showLoading = true
        request(`${baseHost}mpart/oWork/reportDo?action=getResourceByUserId&model=2&pagesize=20&page=${this.loadMorePage}`).then(res => {
          if (!!res.data && res.data.code === 1) {
